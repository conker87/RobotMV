...
public class GroundWaypointsNoSeekAI : EnemyAI {
	...
	protected override void Update() {
		...
		// TODO: Add the ability to allow the EnemyAI to be truely "neutral", which means if you hit it a few times it will turn on you.
		// This AI does not follow the player, it is basically neutral constantly.
		target = null;
		...
		if (target == null) {
			...
			if (EnemyMS == EnemyMovementState.WAITING) {
				...
				// Debug.Log ("EnemyMS == EnemyMovementState.HEADING_HOME && Time.time > waitingTimeNext");
				EnemyMS = EnemyMovementState.IDLE;

			} else if (EnemyMS == EnemyMovementState.IDLE) {

				// Debug.Log ("EnemyMS == EnemyMovementState.IDLE");
		...
		if (Vector3.Distance (transform.position, path.vectorPath [currentWaypoint]) < nextWaypointDistance) {
			...
			if (EnemyMS == EnemyMovementState.PATROLLING && currentWaypoint > path.vectorPath.Count - 1) {
				...
				EnemyMS = EnemyMovementState.WAITING;
